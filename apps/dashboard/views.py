...
_AUTO_CALC_NAME_EN = {r["name_en"] for r in CALC_INDICATORS if r.get("expr")}
...
def save_to_database(rows_data):
    ...
    for row in rows_data:
        ...
        if city_id and year:
            try:
                recompute_pairs.append((int(city_id), int(year), int(province_id or 0)))
        ...
        for group in groups:
            ...
            # 自动计算指标不由手工录入覆盖
            if name_en in _AUTO_CALC_NAME_EN:
                continue
...
def save_df_to_database(rows_data, year, *, background_calc=True):
    ...
    for row in rows_data:
        ...
        province_info = city_code_to_province.get(str(city_id)) or city_code_to_province.get(city_id)
        province_id = int(province_info['province_code']) if province_info else 0
        if city_id:
            city_id = int(city_id)
            recompute_pairs.append((city_id, year, province_id))

...
from apps.coredata.models import Province, City


# 中国 34 个省级行政区 + 所有地级市 / 州 / 盟
# 数据基于国家统计局行政区划代码（到地级层面）
CHINA_REGIONS = [
    {
        "province_name": "北京市",
    ...
    },
    {
        ...
        "cities": [
            ...
            {"name": "金门县", "code": "710500"},
            {"name": "南投县", "code": "710600"},
            {"name": "基隆市", "code": "710700"},
            {"name": "新竹市", "code": "710800"},
            {"name": "嘉义市", "code": "710900"},
            {"name": "新北市", "code": "711100"},
            {"name": "宜兰县", "code": "711200"},
            {"name": "新竹县", "code": "711300"},
            {"name": "桃园市", "code": "711400"},
            {"name": "苗栗县", "code": "711500"},
            {"name": "彰化县", "code": "711700"},
            {"name": "嘉义县", "code": "711900"},
            {"name": "云林县", "code": "712100"},
            {"name": "屏东县", "code": "712400"},
            {"name": "台东县", "code": "712500"},
            {"name": "花莲县", "code": "712600"},
            {"name": "澎湖县", "code": "712700"},
            {"name": "连江县", "code": "712800"},
        ],
    },
    {
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--reset",
            action="store_true",
            help="导入前先清空所有 Province 与 City 数据（慎用，仅建议在开发环境）",
        )
    ...
    def handle(self, *args, **options):
        ...
        if reset:
            self.stdout.write(self.style.WARNING("即将删除所有 Province 与 City 数据..."))
            City.objects.all().delete()
            Province.objects.all().delete()
            self.stdout.write(self.style.SUCCESS("已清空原有省市数据。"))

        created_provinces = 0
        created_cities = 0

        for province_data in CHINA_REGIONS:
            p_name = province_data["province_name"]
            p_code = province_data["province_code"]
            ...
            if p_created:
                created_provinces += 1
                self.stdout.write(self.style.SUCCESS(f"创建省份: {p_name} ({p_code})"))
            else:
                if province.name != p_name:
                    province.name = p_name
                    province.save(update_fields=["name"])
                    self.stdout.write(f"更新省份名称: {p_name} ({p_code})")

            for city_data in province_data.get("cities", []):
                c_name = city_data["name"]
                c_code = city_data["code"]

                city, c_created = City.objects.get_or_create(
                    code=c_code,
                    defaults={"name": c_name, "province": province},
                )
                if c_created:
                    created_cities += 1
                    self.stdout.write(f"  创建城市: {c_name} ({c_code})")
                else:
                    updated = False
                    if city.name != c_name:
                        city.name = c_name
                    ...
                    if city.province_id != province.id:
                        ...
                        updated = True
                    if updated:
                        city.save(update_fields=["name", "province"])
                        self.stdout.write(f"  更新城市: {c_name} ({c_code})")

        self.stdout.write(
            self.style.SUCCESS(
                f"导入完成：新增省份 {created_provinces} 个，新增城市 {created_cities} 个。"
            )
        )